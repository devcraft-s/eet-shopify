...
async function main() {
  try {
    ...
    const eetSkus = new Set(jsonData.products.map(p => p.varenr));
    const shopifyProductsNotInEET = shopifyProducts.filter(shopifyProduct => {
      // Check if any variant of this Shopify product has a SKU that's not in EET
    ...
    if (shopifyProductsNotInEET.length > 0) {
      ...
      for (const product of shopifyProductsNotInEET) {
        try {
          // Get the first variant SKU for logging
          const firstSku = product.variants.nodes[0]?.sku || 'unknown';
          console.log(`Making draft: ${product.title} (${firstSku})`);
          ...
          const draftResponse = await shopifyClient.runGraphQL(draftMutation);
          ...
          if (draftResponse.data.productUpdate.userErrors.length > 0) {
            ...
            console.log(`❌ Failed to make draft: ${product.title} - ${errors.map(e => e.message).join(', ')}`);
          } else {
            draftSuccessCount++;
            ...
            console.log(`✅ Made draft: ${product.title}`);
          }
          ...
          console.log(`❌ Error making draft: ${product.title} - ${error.message}`);
        }
      }
